...
int create_retinaface(char *model_name, rknn_context *ctx, int &width, int &height, int &channel, std::vector<float> &out_scales, std::vector<int32_t> &out_zps, rknn_input_output_num &io_num, unsigned char *model_data)
{
    ...
    LOGE("sdk version: %s driver version: %s\n", version.api_version, version.drv_version);
    ...
    LOGE("model input num: %d, output num: %d\n", io_num.n_input, io_num.n_output);
  	...
  	if (input_attrs[0].fmt == RKNN_TENSOR_NCHW) {
        LOGE("model is NCHW input fmt\n");
		channel = input_attrs[0].dims[1];
		width   = input_attrs[0].dims[2];
		height  = input_attrs[0].dims[3];
  	} else {
        LOGE("model is NHWC input fmt\n");
		width   = input_attrs[0].dims[1];
		height  = input_attrs[0].dims[2];
		channel = input_attrs[0].dims[3];
  	}
    LOGE("model input height=%d, width=%d, channel=%d\n", height, width, channel);
...
int retinaface_output_release(rknn_context *ctx, rknn_input_output_num io_num, rknn_output *outputs)
{
    int ret;
