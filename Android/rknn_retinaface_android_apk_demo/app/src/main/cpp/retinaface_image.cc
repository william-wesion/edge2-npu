...
int create(int im_height, int im_width, int im_channel, char *model_path)
{
    ...
    for (int i = 0; i < n_output; ++i) {
        if (i == 1) {
            // 4.2.1 Create output tensor memory, output data type is float, post_process need int8 data.
...
int retinaface_post_process(char *grid0_buf, float *grid1_buf, char *grid2_buf,
    ...
    ret = post_process((int8_t *)grid0_buf, (float *)grid1_buf, (int8_t *)grid2_buf,
    ...
    for (int i = 0; i < count; ++i) {
        ...
        boxes[4 * i + 0] = det_result->box.left;
        boxes[4 * i + 1] = det_result->box.top;
        boxes[4 * i + 2] = det_result->box.right;
        boxes[4 * i + 3] = det_result->box.bottom;

        points[i * 10 + 0] = det_result->point.point_1_x;
        points[i * 10 + 1] = det_result->point.point_1_y;
        points[i * 10 + 2] = det_result->point.point_2_x;
        points[i * 10 + 3] = det_result->point.point_2_y;
        points[i * 10 + 4] = det_result->point.point_3_x;
        points[i * 10 + 5] = det_result->point.point_3_y;
        points[i * 10 + 6] = det_result->point.point_4_x;
        points[i * 10 + 7] = det_result->point.point_4_y;
        points[i * 10 + 8] = det_result->point.point_5_x;
        points[i * 10 + 9] = det_result->point.point_5_y;

//        LOGI("%s @ (%d %d %d %d) %f\n", det_result->name, det_result->box.left, det_result->box.top,det_result->box.right, det_result->box.bottom, det_result->prop);
//        LOGI("P%d (%d %d)\n", 1, det_result->point.point_1_x, det_result->point.point_1_y);
//        LOGI("P%d (%d %d)\n", 2, det_result->point.point_2_x, det_result->point.point_2_y);
//        LOGI("P%d (%d %d)\n", 3, det_result->point.point_3_x, det_result->point.point_3_y);
//        LOGI("P%d (%d %d)\n", 4, det_result->point.point_4_x, det_result->point.point_4_y);
//        LOGI("P%d (%d %d)\n", 5, det_result->point.point_5_x, det_result->point.point_5_y);
